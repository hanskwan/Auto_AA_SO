...
def not_equity(listl):
    for i in listl:
        if i >9999:
            print(i + " is not a common stock, please delete from the list")
        else:
            pass

not_equity(stock_list)

# error_list = []

### functions for return button coordinatein number
    
def findloc_tic_test(picname):
...
ticx, ticy = findloc_tic_test("DQL") # DQL is the picture above the stock code input position

def findloc_SO_test(picname):
    pic = pg.locateOnScreen(str(picname) + '.png')
    pic_mid = pg.center(pic)
    pic_midx, pic_midy = pic_mid
    adj_x = pic_midx/2
...
SOx, SOy = findloc_SO_test("SO") # SO is the picture at the right of the issued Captial number
...
for i,j in enumerate(stock_list):
    ...
    while text[-1].isdigit() == True and text[0].isdigit() == True:
        break
    else:
        time.sleep(0.7)
        pg.moveTo(SOx,SOy) # SO position
...
res1=[]
for i in res:
    API = "=BDP(\"" + str(i) + "\", " "\"EQY_SH_OUT_TOT_MULT_SH\")"
    res1.append(API)
...
res2=[]
for i in stock_list:
    filter_1 = "=if(ABS(C2-D2*1000000)>1000000 ,1 ,0)"
